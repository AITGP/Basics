...
BEGIN
  -- Look for the ACL currently assigned to '*' and give SACSAUSR
  -- the "connect" privilege if SACSAUSR does not have the privilege yet.
...
DECLARE
  ACL_PATH  VARCHAR2(4000);
BEGIN
  -- Look for the ACL currently assigned to 'localhost' and give SACSAUSR
  -- the "connect" privilege if SACSAUSR does not have the privilege yet.
  SELECT ACL INTO ACL_PATH FROM DBA_NETWORK_ACLS
   WHERE HOST = 'localhost' AND LOWER_PORT IS NULL AND UPPER_PORT IS NULL;
   
  IF DBMS_NETWORK_ACL_ADMIN.CHECK_PRIVILEGE(ACL_PATH, :SCHEMA_NAME,
...
---Once the ACL has been fixed, you must run the first script in this section to apply the ACL to the SACSAUSR user. See "Granting Connect Privileges".